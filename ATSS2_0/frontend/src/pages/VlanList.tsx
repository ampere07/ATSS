...
const VlanList: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const refreshData = async () => {
      console.log('User idle for 15 minutes, auto-refreshing VLAN data...');
      try {
        await loadVlans(true); // Silent refresh
      } catch (err) {
  ...
  const handleDelete = (vlan: Vlan) => {
    showGlobalModal(
      'confirm',
      'Confirm Deletion',
      `Are you sure you want to permanently delete VLAN "${vlan.value}"?`,
      () => executeDelete(vlan)
    );
  };

  const executeDelete = async (vlan: Vlan) => {
    closeGlobalModal();
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        showGlobalModal('success', 'Deleted', data.message || 'VLAN deleted successfully');
      } else {
        showGlobalModal('error', 'Delete Failed', data.message || 'Failed to delete VLAN');
      }
    } catch (error: any) {
  ...
  const filteredVlans = vlans.filter(vlan => {
    ...
    const matchesOrg = currentUserOrgId
      ? vlan.organization_id === currentUserOrgId
      : !vlan.organization_id;

    if (!matchesOrg) return false;

    const query = searchQuery.toLowerCase();
    return String(vlan.value).toLowerCase().includes(query);
  });

  const totalPages = Math.ceil(filteredVlans.length / itemsPerPage);
  const paginatedVlans = filteredVlans.slice(
      ...
      >
        <div className="flex-1 min-w-0 pr-4">
          ...
          <div className={`flex items-center gap-4 mt-1 text-[10px] uppercase font-medium ${isDarkMode ? 'text-gray-500' : 'text-gray-400'
            ...
            <span>Created: {formatDate(vlan.created_at)}</span>
            <span>Updated: {formatDate(vlan.updated_at)}</span>
          </div>
        </div>
    ...
    <div className={`h-full flex flex-col overflow-hidden ${isDarkMode ? 'bg-gray-950 text-white' : 'bg-gray-50 text-gray-900'
      ...
      <div className="flex-1 overflow-auto custom-scrollbar">
        {isLoading && vlans.length === 0 ? (
          ...
          <div>
            {paginatedVlans.map(renderListItem)}
          </div>
        ) : (
      ...
      {/* Pagination Controls */}
      {!isLoading && filteredVlans.length > 0 && totalPages > 1 && (
        <div className={`border-t p-4 flex items-center justify-between ${isDarkMode ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200 shadow-lg'}`}>
          <div className={`flex items-center gap-4 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            ...
            <div>
              Showing <span className="font-medium">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="font-medium">{Math.min(currentPage * itemsPerPage, filteredVlans.length)}</span> of <span className="font-medium">{filteredVlans.length}</span> results
            </div>
          </div>
