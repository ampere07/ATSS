...
interface CommissionState {
    ...
    // The Incentives and Bonus tabs on the History screen. These were absent
    // from this store while the page already rendered tabs for them, so both
    // read `(store as any).incentiveHistory ?? []` and were permanently empty
    // however much the agent had earned. Mirrors the web store.
...
export const useCommissionStore = create<CommissionState>((set, get) => ({
    ...
    fetchCommissions: async (force = false) => {
        ...
        try {
            ...
            // 3. Fetch Incentive History progressively
            ...
            while (hasMoreIncentives) {
                ...
                if (res.success) {
                    ...
                    const totalServer = res.total || 0;
                    set({ incentiveHistory: [...allIncentives], totalIncentives: totalServer });
            ...
            // 4. Fetch Bonus History progressively
            let allBonus: any[] = [];
            let bonusOffset = 0;
            ...
            while (hasMoreBonus) {
                ...
                if (res.success) {
                    ...
                    const totalServer = res.total || 0;
                    set({ bonusHistory: [...allBonus], totalBonus: totalServer });
    ...
    fetchUpdates: async () => {
        ...
        try {
            ...
            if (incentiveRes.success && (incentiveRes.data?.length ?? 0) > 0) {
                const updates = incentiveRes.data;
                set((state) => {
                    const map = new Map(state.incentiveHistory.map((i: any) => [i.id, i]));
                    updates.forEach((u: any) => map.set(u.id, u));
                    return {
                        incentiveHistory: Array.from(map.values()).sort((a: any, b: any) =>
                            new Date(b.processed_at || b.created_at || 0).getTime()
                            - new Date(a.processed_at || a.created_at || 0).getTime()),
                        totalIncentives: incentiveRes.total || get().totalIncentives
                    };
                });
            }

            if (bonusRes.success && (bonusRes.data?.length ?? 0) > 0) {
                const updates = bonusRes.data;
                set((state) => {
                    const map = new Map(state.bonusHistory.map((i: any) => [i.id, i]));
                    updates.forEach((u: any) => map.set(u.id, u));
                    return {
                        bonusHistory: Array.from(map.values()).sort((a: any, b: any) =>
                            new Date(b.created_at || 0).getTime() - new Date(a.created_at || 0).getTime()),
                        totalBonus: bonusRes.total || get().totalBonus
                    };
                });
            }
