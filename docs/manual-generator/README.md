...
| SYNC Roles Module User Manual — Role Management, permissions and access | `../SYNC_Roles_Module_Complete_User_Manual.pdf` | `roles_module_manual.js` |
...
## Regenerate the Roles Module manual

```sh
cd docs/manual-generator
node roles_module_manual.js            # writes ../SYNC_Roles_Module_Complete_User_Manual.pdf
```

Takes the same optional arguments as the others: output path, date, version, prepared-by,
organization.

Hand-written. It documents Role Management as an administrator works it: the role list and its
badges, every field on the role form, the permission grid and the four rules it follows while it
is ticked, roles built on top of a system role, assigning a role to a person, and every page and
button-level permission that can be granted.

Two rules when editing it:

- **Name things as the screen names them.** The grid shows page labels (`Agent Invoices`) and
  button labels (`Set Status`), never the underlying keys. The manual has to match what the
  administrator is looking at.
- **The page list changes with the product.** When a page or an action is added to the permission
  catalog, add it to the reference chapter — chapter 14 is the only place it is enumerated.
...
| `roles_module_manual.js` | The SYNC Roles Module User Manual. Hand-written, covering Role Management, the role form, the permission grid and everything that can be granted. |
