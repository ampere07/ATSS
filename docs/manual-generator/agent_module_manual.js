...
const VERSION = process.argv[4] || '1.1';
...
doc.p(
  'Every screen in both applications is governed by a permission, and every request either '
  + 'application makes is checked against that same permission on the server. The menus only '
  + 'decide what to draw; the server decides what is allowed. Hiding a button is a convenience, '
  + 'never the protection — an agent who reached an administrator’s screen another way would '
  + 'still be refused the moment they tried to use it.'
);

doc.h2('6.2 What the Agent Role Holds');
doc.p(
  'An account on the built-in Agent role holds exactly these screens, and nothing else. The names '
  + 'are the ones that appear in Role Management.'
);
doc.table(['Screen', 'What the agent gets'], [
  ['Agent Dashboard', 'The agent’s landing screen.'],
  ['Agent Application Form', 'The customer application form.'],
  ['Job Order', 'Job Order, filtered by both the server and the app to the agent’s own referrals.'],
  ['Work Order', 'Work Order.'],
  ['Bonus History',
   'The agent’s own payout, incentive and bonus history. It is listed as "History" in both '
   + 'menus.'],
  ['Agent Invoices',
...
doc.table(['Withheld', 'Consequence'], [
  ['Payout, on Bonus History',
   'The Add button on the History screen is not drawn, and a payout recorded by an agent is '
   + 'refused. Agents cannot pay themselves.'],
  ['The Agent Payout screen, and its Approve',
   'No access to Agent Payout, and no ability to approve or reject any payout.'],
  ['Generate, on Agent Invoices', 'The agent can read invoices but cannot generate them.'],
  ['Set Status, on Agent Invoices',
   'The agent sees each invoice’s status as a read-only badge rather than a dropdown.'],
  ['Agent Management and Team Agents', 'No access to agent accounts or teams.'],
], { widths: [32, 68] });
...
doc.p(
  'Roles beyond the eight built-in ones are created in Role Management, under Users > Roles, and '
  + 'carry their own ticked list of screens and buttons. To build an agent-like role, tick the six '
  + 'screens in 6.2. To build a restricted administrator who runs the agent programme, tick Agent '
  + 'Payout with Approve, Bonus History with Payout, Agent Management, Team Agents, and Agent '
  + 'Invoices with Generate and Set Status, as the job requires.'
);
doc.p(
  'A quicker route for that second one is to start the custom role from the Administrator role '
  + 'and tick only the extras: it then keeps step with Administrator as the product grows, instead '
  + 'of freezing a copy of it.'
);
doc.p(
  'The Mobile Application refreshes a signed-in user’s permissions once per launch, so a role '
  + 'edited while somebody is signed in takes effect on their next launch. The Web Application '
  + 'applies it on their next action.'
);
doc.callout('Roles are documented in full in their own manual',
  'The SYNC Roles Module User Manual covers Role Management end to end — every field on the role '
  + 'form, the permission grid and the rules it follows, building a role on top of a built-in one, '
  + 'and every screen and button that can be granted.');
...
doc.steps([
  ...
  'It returns the account’s identity, its role, everything that role is allowed to reach, '
  + 'and the screen the role should land on.',
...
doc.p('Every invoice list is narrowed before it is read, on the server, on every screen:');
...
doc.callout('An agent cannot reach another team’s invoice',
  'The same narrowing is applied everywhere invoices are read, so guessing an invoice number or '
  + 'an address is refused with "Invoice not found" rather than returning somebody else’s '
  + 'document.');
...
doc.h2('21.2 How to Tell a Run Has Happened');
doc.p(
  'You do not have to take any of it on trust. Each of these jobs leaves something visible on a '
  + 'screen you already use, so a run can be confirmed without asking anybody.'
);
doc.table(['To confirm', 'Look at', 'What you should see'], [
  ['This week’s invoices were raised', 'Agent > Invoices',
   'One invoice per team and per solo agent, dated the Monday, covering the week that just '
   + 'ended.'],
  ['An agent’s incentive was awarded', 'Agent Payout, or the agent’s own History screen',
   'A new incentive entry, and the incentive figure on the balance moved up by the incentive '
   + 'value.'],
  ['Quota progress is being counted', 'The agent’s Dashboard on the Mobile Application',
   'Progress toward the next incentive, counting up as referrals complete. It never resets '
   + 'partway.'],
  ['An achievement period closed', 'The agent’s Dashboard achievement cards',
   'The period that ended shown with what it finished on, and a fresh period started.'],
], { widths: [26, 30, 44] });

doc.h2('21.3 If Something Looks Like It Did Not Run');
doc.p(
  'Check the visible things first — they explain most of it. An invoice is not raised for an '
  + 'owner with nothing billable that week; an incentive is not awarded until a whole quota is '
  + 'complete, and a part-finished quota simply carries forward; and an agent with no quota or no '
  + 'incentive value set on their account earns no incentive at all until somebody fills those in '
  + '(see 2.1).'
);
doc.p(
  'If all of that is right and the figure is still not there, ask your systems administrator. '
  + 'They have a read-only check that walks exactly the same steps the system walked for one '
  + 'particular referral and explains each one, without changing anything, and it answers the '
  + 'commonest question in this module — why one job order did or did not settle a commission '
  + 'with its referring agent.'
);
...
doc.table(['Problem', 'Likely cause', 'What to do'], [
  ['A menu entry is missing',
   'The role does not hold that screen.',
...
doc.table(['Problem', 'Likely cause', 'What to do'], [
  ...
  ['Approve and Reject are not shown',
   ...
   + 'permission.',
   'Payouts are approved in the Web Application only. There, a Pending record shows both buttons '
   + 'to an account whose role holds Approve on Agent Payout.'],
